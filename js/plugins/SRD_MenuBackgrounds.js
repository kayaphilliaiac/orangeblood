...
Imported["SumRndmDde Menu Backgrounds"] = 2.01;
...
for(let i = 1; i <= 20; i++) {
	...
	if(scene.trim().length > 0) {
		...
		_.opacity[scene] = String(params['Custom ' + i + ' Opacity']);
...
Scene_MenuBase.prototype.create = function() {
	...
	if(_.opacity[this._menuBackgroundConstructorName]) {
		this._windowLayer.children.forEach(function(window) {
			window.opacity = _.opacity[this._menuBackgroundConstructorName];
		}, this);
	}
};
...
Scene_MenuBase.prototype.createBackground = function() {
	//Compatibility with Yanfly's Party System cause the Scene_Party has no constructor name? D:
	if(this._formerParty) this._menuBackgroundConstructorName = 'Scene_Party';
