...
Imported["SumRndmDde Smooth Camera"] = 1.00;
...
Game_Map.prototype.setDisplayPosStart = function(x, y, dur) {
	if(this._currentFocusEvent !== $gameScreen.focusEvent) {
		this._currentFocusEvent = $gameScreen.focusEvent;
...
Game_Character.prototype.updateScroll = function() {
	if(!$gameMap.isCameraScrolling()) {
		const cut = _.cutOff;
		const xSpeed = Math.floor(this.screenX() - $gameScreen.zoomX()) * _.power;
		const ySpeed = Math.floor(this.screenY() - $gameScreen.zoomY()) * _.power;
		if(xSpeed < -cut) {
