...
Imported['ItemSort'] = 1.01;

/*:
 * @plugindesc ver1.01/アイテムをソートする仕組みを追加します。
 * @author Yana
 *
 * @param SortTrigger
...
(function(){
    ...
    Game_System.prototype.pushNewerItems = function(item) {
        ...
        if (this._newerItems.contains(item)) return;
        this._newerItems.unshift(item);
    ...
    Window_ItemList.prototype.drawItem = function(index) {
        ...
        if (this.isHighlightNewItem() && item && newerItemColor1 && $gameSystem.newerItems().contains(item)) {
        ...
        if (this.isHighlightNewItem() && newerItemNewText && item && $gameSystem.newerItems().contains(item)) {
    ...
    if (newerItemFontColor) {
        Window_ItemList.prototype.drawItemName = function (item, x, y, width) {
            ...
            var isNewer = this.isHighlightNewItem() && item && newerItemColor1 && $gameSystem.newerItems().contains(item);
            width = width || 312;
            if (item) {
    ...
    Window_ItemList.prototype.sortNewer = function(data) {
      data.sort(function(a,b){
          var ai = $gameSystem.newerItems().indexOf(a) + 1;
          var bi = $gameSystem.newerItems().indexOf(b) + 1;
    ...
    Window_ItemList.prototype.isSortEnabled = function() {
        ...
        return this._sortEnabled && (this.isOpenAndActive() || (cw && cw.isOpen()));
    };
